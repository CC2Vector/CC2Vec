...
ROOT_PATH = './data/1/'
TRAIN_FILE_PATH = ROOT_PATH + 'train_data.csv'
TRAIN_WEIGHT_FILE_PATH = ROOT_PATH + 'train_weight.csv'
...
model = network_no_siamese(weights).to(device)
criterion = nn.MSELoss().to(device)
# optimizer = torch.optim.Adam(model.parameters(), lr = 0.005)
optimizer = torch.optim.RMSprop(filter(lambda x: x.requires_grad, model.parameters()), lr=LEARNING_RATE)
...
for epoch in range(num_epochs):
    ...
    for i, (q1_batch, q1_batch_lengths, q2_batch, q2_batch_lengths, labels, w1_batch, w2_batch) in enumerate(
        ...
        loss = criterion(similarity_score, net_labels)
